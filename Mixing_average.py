...
for j in range(runs):
    print j
    sites = [[p/40-1 for p in range (120)] for q in range (80)] # a matrix to recod whether a site is occupied by a molecule of A or B
...
np.savetxt('sumA_2.txt',sumA)
np.savetxt('sumB_2.txt',sumB)
...
pl.savefig('average_densities_try.pdf') 
pl.show()

...
def Diffusion(r,m,x,y,sites1,sites2,mol):# where r is a random number from 0 to N-1, namely a random particle
    ...
    if m == 1 and sites1[y+1,x] == 0 and sites2[y+1,x] == 0: #decide whether a site is occupied already 
        sites1[y+1,x] = 1 #update the occupied sites
        sites1[y,x] = 0
        mol[r,1] = mol[r,1]+1 # move the molecule
    if m == 2 and sites1[y-1,x] == 0 and sites2[y-1,x] == 0: #decide whether a site is occupied already
        sites1[y-1,x] = 1 #update the occupied sites
        sites1[y,x] = 0
        mol[r,1] = mol[r,1]-1 # move the molecule
    if m ==3 and sites1[y,x-1] == 0 and sites2[y,x-1] == 0: #decide whether a site is occupied already
        sites1[y,x-1] = 1 #update the occupied sites
        sites1[y,x] = 0
        mol[r,0] = mol[r,0]-1 # move the molecule
    if m ==4 and sites1[y,x+1] == 0 and sites2[y,x+1] == 0: #decide whether a site is occupied already
        sites1[y,x+1] = 1 #update the occupied sites
        sites1[y,x] = 0
        mol[r,0] = mol[r,0]+1 # move the molecule
...
pl.xlim(0,119)
pl.ylim(0,79)
pl.title('Mixing of two Gases, t=50000000')
pl.xlabel('x')
pl.ylabel('y')
